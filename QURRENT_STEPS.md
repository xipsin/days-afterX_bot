...
## Итерация 7: Архитектурный рефакторинг и система доступа (15.11.2025)

**Цель**: Реализовать команду `/add` с FSM, отполировать UX, внедрить надежную систему аутентификации и разделить хэндлеры по функциональности.

### Итерация 7.0 -> 7.2: Рефакторинг и отладка FSM
- **Проблема**: "Вшитые" строки в коде, баги с `datetime`, некорректная работа FSM с инлайн-кнопками.
- **Решение**: Вынесли все строки в `lexicon_ru.py`, исправили `naive/aware` datetime, починили FSM, передавая `callback.from_user`.
- **Файлы (v5.x):** `user_handlers.py`, `lexicon_ru.py`, `inline.py`.

### Итерация 7.3: Архитектурный прорыв
- **Проблема**: Любой пользователь, даже незарегистрированный, мог инициировать команды. Это фундаментальная дыра в безопасности.
- **Решение**:
    1.  Разработана и утверждена архитектура с разделением роутинга на `public_router` и `private_router`.
    2.  Создан `AuthMiddleware` для проверки регистрации пользователя в базе данных.
    3.  Middleware "навешано" только на `private_router`, оставляя `/start` общедоступным.
    4.  Старый `user_handlers.py` разделен на `common_handlers.py` и `event_handlers.py`.
- **Новые файлы**: `src/middlewares/auth.py`, `src/handlers/common_handlers.py`, `src/handlers/event_handlers.py`.
- **Статус**: **ЗАВЕРШЕНО УСПЕШНО**. Архитектура реализована, все тесты пройдены.

---
**Версии файлов на конец итерации:**
- `src/main.py`: v3.1
- `src/handlers/common_handlers.py`: v1.0
- `src/handlers/event_handlers.py`: v1.0
- `src/middlewares/auth.py`: v1.0
- `src/lexicon/lexicon_ru.py`: v1.3
- `src/keyboards/inline.py`: v2.3



## Итерация 8: Реализация просмотра событий (`/list`) (15.11.2025)

**Цель**: Дать пользователю возможность просматривать список своих событий с помощью команды `/list`.

- **Решение**:
    1.  Создан `DbSessionMiddleware` для автоматического предоставления сессии БД в хэндлеры.
    2.  Создан `utils/formatters.py` с функцией `format_days_passed` для корректного склонения дней.
    3.  В `event_handlers.py` добавлен хэндлер для команды `/list`, который формирует нумерованный список событий.
    4.  В `keyboards/inline.py` добавлена функция `get_list_actions_keyboard` для создания инлайн-клавиатуры с действиями.
    5.  Клавиатура выводится вместе со списком событий, кнопки расположены вертикально для лучшей читаемости.
    6.  Добавлены хэндлеры-заглушки для кнопок "Сбросить" и "Удалить".
    7.  Весь лексикон переведен на `HTML` для корректного парсинга.
- **Статус**: **ЗАВЕРШЕНО УСПЕШНО**.

---
**Версии файлов на конец итерации:**
- `src/main.py`: v3.2
- `src/handlers/event_handlers.py`: v1.2
- `src/keyboards/inline.py`: v2.5
- `src/lexicon/lexicon_ru.py`: v1.6
- `src/middlewares/db.py`: v1.0
- `src/utils/formatters.py`: v1.0
