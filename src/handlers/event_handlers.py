...
from src.keyboards.inline import get_date_selection_keyboard, get_cancel_keyboard
from src.lexicon.lexicon_ru import LEXICON_RU

from src.middlewares.auth import AuthMiddleware # <-- Важный импорт

# Роутер для команд, связанных с событиями.
# Middleware будет применяться к нему в main.py
router = Router()

# --- ПРИМЕНЯЕМ MIDDLEWARE ПРЯМО К РОУТЕРУ ---
...
async def _process_quick_add(message: Message):
    ...
    await _validate_and_create_event(message, message.from_user, event_name, event_date)

async def _validate_and_create_event(message: Message, user: TgUser, name: str, date: datetime, state: FSMContext = None):
...
async def process_add_command(message: Message, state: FSMContext):
    ...
    if command_text:
        await _process_quick_add(message)
    else:
...
async def process_date_button_fsm(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    await _validate_and_create_event(callback.message, callback.from_user, event_name, event_date, state)

@router.message(AddEvent.waiting_for_date)
async def process_date_text_fsm(message: Message, state: FSMContext):
    ...
    user_data = await state.get_data()
    event_name = user_data.get("event_name")
    await _validate_and_create_event(message, message.from_user, event_name, parsed_date, state)
